...
class EventHandler(AssistantEventHandler):
    ...
    def on_tool_call_created(self, tool_call):
        # Display a message when a tool call is created
        # You might want to customize this based on the tool_call details
        message = f"Received tool call: {tool_call.type}\n\n"
        st.session_state.assistant_text[-1] += message
        st.session_state.text_boxes[-1].info(
            "".join(st.session_state["assistant_text"][-1])
        )

        #        st.chat_message(message, avatar=avatar["assistant"])
        # Handle specific tool calls
        # Check the type of the tool_call and handle accordingly
        if tool_call.type == "function":
            # Now safe to check for function_name as we're dealing with a function tool call
            if tool_call.function_name == "send_email":
                # Ensure to safely access arguments with proper handling
                self.handle_send_email(json.loads(tool_call.function.arguments))
